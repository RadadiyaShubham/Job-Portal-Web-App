...
const Talents = () => {
    ...
    useEffect(() => {
        ...
        if (filter.Skills && filter.Skills.length > 0) {
            filterTalent = filterTalent.filter((talent: any) => filter.Skills?.some((skills: any) => talent?.skills?.some((talentSkill:any)=>talentSkill.toLowerCase().includes(skills.toLowerCase()))));
        }
    ...
    return <div className="p-5">
        ...
        <div className="mt-10 flex flex-wrap gap-5 justify-center">
            {
                filteredTalents?.length?filteredTalents.map((talents: any, index: any) => <TalentCard key={index} {...talents} />):<div className="text-xl font-semibold">No Talents found</div>
            }
        </div>
    </div>
}
